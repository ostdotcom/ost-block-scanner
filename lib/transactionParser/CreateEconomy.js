...
const rootPrefix = '../..',
  ...
  contractConversionFactors = require(rootPrefix + '/config/genericData/contractConversionFactors.json'),
...
class CreateEconomy {
  ...
  async updateIntoEconomy(contractDetails) {
    ...
    let totalSupply = contractDetails.totalSupply || '0',
      ...
      conversionFactor =
        oThis.conversionFactor || contractConversionFactors[oThis.contractAddress] || sanitizer.sanitizeNumber(1),
      tokenDisplayName = oThis.displayName || contractDetails.name || '',
      tokenDisplaySymbol = oThis.symbol || contractDetails.symbol || '',
    ...
    if (oThis.originContractAddress) {
      insertParams['originContractAddress'] = oThis.originContractAddress;
    }

    if (oThis.simpleStakeAddress) {
      insertParams['simpleStakeAddress'] = oThis.simpleStakeAddress;
    }

    // If economy was updated, then somethings would not be updated in db.
    if (oThis.isUpdate === 0) {
      insertParams['name'] = (contractDetails.name || oThis.contractAddress).toLowerCase();
      insertParams['displayName'] = contractDetails.name || oThis.contractAddress;
