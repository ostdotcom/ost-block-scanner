...
class CreateEconomy {
  ...
  async updateIntoEconomy(contractDetails) {
    ...
    if (oThis.isUpdate === 0) {
      insertParams['name'] = (oThis.displayName || contractDetails.name || oThis.contractAddress).toLowerCase();
      insertParams['displayName'] = oThis.displayName || contractDetails.name || oThis.contractAddress;
