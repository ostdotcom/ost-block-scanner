...
class AddTransactions {
  ...
  async asyncPerform() {
    ...
    if (oThis.newTransactionsInserted.length > 0) {
      ...
      // If data source for transaction details is other than Dynamo then no need to insert in table.
      ...
      if (transactionDetailsDataSource == '') {
  ...
  _createTransactionInsertParams(transactionReceipt, transactionAttributes) {
    ...
    if (transactionReceipt.hasOwnProperty('status')) {
      transactionStatus = basicHelper.getTransactionStatusForDB(transactionReceipt.status);
    }
    if (transactionReceipt.hasOwnProperty('internalStatus')) {
      internalStatus = transactionReceipt.internalStatus;
    }
    ...
    for (let longNameAttr in transactionAttributes) {
      // if attr already there in data to insert map, nothing to do
      if (dataToInsert.hasOwnProperty(longNameAttr)) continue;

      // if failed, insert transfers into transactions
      if (longNameAttr === 'transfers') {
        // preserve transfers only in case of failed transaction
        if (!(transactionStatus && internalStatus)) dataToInsert[longNameAttr] = transactionReceipt[longNameAttr] || '';
      } else {
        dataToInsert[longNameAttr] = transactionReceipt[longNameAttr] || '';
      }
    }

    dataToInsert['eventsParsingStatus'] = 0;

    if (transactionReceipt && transactionReceipt.logs && transactionReceipt.logs.length) {
      let formatTransactionLogsObj = new formatTransactionLogs(transactionReceipt);
      let logEventsArray = formatTransactionLogsObj.getLogsArrayToDecode(transactionReceipt.logs).parsableEvents;
      dataToInsert['eventsParsingStatus'] = logEventsArray.length ? 1 : 0;
    }
  ...
  _createTransactionExtendedInsertParams(transactionReceipt, transactionDetailsAttributes) {
    ...
    for (let attr in transactionDetailsAttributes) {
      if (!transactionDetailsInsertMap[attr]) {
          ...
          trxModelObj
            ...
            .then(function(resp) {
              if (!resp || resp.isFailure()) {
                ...
                oThis._setInsertedTransactionsCache(shardId, insertParams[shardId]);
              }
              onResolve();
            })
            .catch(function(err) {
          ...
          trxExtModelObj
            ...
            .then(function(resp) {
              if (!resp || resp.isFailure()) {
                ...
                oThis._setInsertedTransactionsDetailsCache(shardId, insertParams[shardId]);
              }
              onResolve();
            })
            .catch(function(err) {
  ...
  /**
   * Set Cache of transactions which are inserted in DB.
   *
   * @param shardId
   * @param insertedTransactionsData
   * @returns {Promise<void>}
   * @private
   */
  async _setInsertedTransactionsCache(shardId, insertedTransactionsData) {
    const oThis = this,
      trxModelKlass = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'TransactionModel');

    for (let i = 0; i < insertedTransactionsData.length; i++) {
      let trxModelObj = new trxModelKlass({ chainId: oThis.chainId, shardIdentifier: shardId }),
        insertedData = insertedTransactionsData[i];

      await trxModelObj.setCache(insertedData);
    }
  }

  /**
   * Set Cache of transactions details which are inserted in DB.
   *
   * @param shardId
   * @param insertedTransactionsData
   * @returns {Promise<void>}
   * @private
   */
  async _setInsertedTransactionsDetailsCache(shardId, insertedTransactionsData) {
    const oThis = this,
      trxModelKlass = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'TransactionDetailModel');
