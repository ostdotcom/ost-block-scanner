...
class FormatTransactionLogs {
  ...
  constructor(transactionReceipt) {
    ...
    oThis.transactionReceipt.internalStatus = basicHelper.getTransactionStatusForDB(oThis.transactionReceipt.status);
  }
  ...
  perform() {
    ...
    if (oThis.transactionReceipt && oThis.transactionReceipt.logs && oThis.transactionReceipt.logs.length) {
      ...
      if (logs.length) {
        let abiDecoder = new abiDecoderKlass();
        abiDecoder.initParseEventAbi();
  ...
  /**
   * Decode Rule executed event from transaction log.
   *
   * @returns {*|result}
   */
  decodeRuleExecuteEvent() {
    const oThis = this;

    if (oThis.transactionReceipt && oThis.transactionReceipt.logs && oThis.transactionReceipt.logs.length) {
      let logs = oThis.transactionReceipt.logs;
      for (let i = 0; i < logs.length; i++) {
        let txReceiptLogElement = logs[i];
        if (txReceiptLogElement.topics && txReceiptLogElement.topics.length) {
          let eventSignature = txReceiptLogElement.topics[0];
          if (eventSignature == eventConstants._ruleExecutedEventSignature) {
            let abiDecoder = new abiDecoderKlass();
            abiDecoder.initRuleExecutedEventAbi();
    ...
    if (eventData && eventData.events && eventData.events.length > 0) {
      for (let j = 0; j < eventData.events.length; j++) {
        ...
        if (eventVal.name === '_status') {
          oThis.transactionReceipt.internalStatus = basicHelper.getTransactionStatusForDB(eventVal.value);
        }
      }
    }
  }
