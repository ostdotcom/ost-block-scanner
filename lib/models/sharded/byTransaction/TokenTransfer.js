...
class TokenTransferModel extends ShardedBase {
  ...
  /**
   * Set Cache of Token Transfer Data
   *
   * @param dbResponse
  ...
  async setCache(dbResponse) {
    const oThis = this,
      cacheKlass = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'TokenTransferCache'),
      cacheObj = new cacheKlass({
        chainId: dbResponse.chainId,
        transactionHashEventIndexesMap: { [dbResponse.transactionHash]: [dbResponse.eventIndex] }
      });

    await cacheObj.setCache(`${dbResponse.transactionHash}-${dbResponse.eventIndex}`, dbResponse);
  }
  ...
  async afterUpdate(updatedData) {
    ...
    new AddressBalanceCacheClass({
      chainId: updatedData.chainId,
      economyContractAddress: updatedData.contractAddress,
      addresses: [updatedData.fromAddress, updatedData.toAddress]
    }).clear();
  }
