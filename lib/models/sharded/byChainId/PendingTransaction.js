...
class PendingTransactionModel extends ShardedBase {
  ...
  async updateItem(dataToUpdate) {
    ...
    const formattedResponse = oThis._formatRowFromDynamo(updateQueryRsp.data.Attributes);
  ...
  async setCache(dbResponse) {
    ...
    const PendingTransactionByHashCache = oThis
        .ic()
        .getShadowedClassFor(coreConstants.icNameSpace, 'PendingTransactionByHashCache'),
      pendingTransactionByHashObj = new PendingTransactionByHashCache({
        chainId: dbResponse.chainId,
        transactionHashes: [dbResponse.transactionHash]
      });

    promises.push(
      pendingTransactionByHashObj.setCache(dbResponse.transactionHash, { transactionUuid: dbResponse.transactionUuid })
    );

    const PendingTransactionByUuidCache = oThis
        .ic()
        .getShadowedClassFor(coreConstants.icNameSpace, 'PendingTransactionByUuidCache'),
      pendingTransactionByUuidObj = new PendingTransactionByUuidCache({
        chainId: dbResponse.chainId,
        transactionUuids: [dbResponse.transactionUuid]
      });

    promises.push(pendingTransactionByUuidObj.setCache(dbResponse.transactionUuid, dbResponse));
