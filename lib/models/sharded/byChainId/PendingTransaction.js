...
class PendingTransactionModel extends ShardedBase {
   ...
   *
   * @param {Array} keys
   * eg: [{"txuuid":{"S":"2w338423912345"},"cid":{"N":"1000"}},{"txuuid":{"S":"2w3384239"},"cid":{"N":"1000"}}]
   *
   * @returns {Promise<>}
   */
  async getPendingTransactionData(keys) {
    const oThis = this;

    let params = [];
    for (let index in keys) {
      params.push(oThis._keyObj(keys[index]));
    }
   ...
   * @returns {Promise<*>}
   */
  async getPendingTransactionsWithHashes(chainId, transactionHashes) {
    const oThis = this,
      promiseArray = [],
      PendingTransactionModel = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'PendingTransactionModel'),
      pendingTransactionModelObject = new PendingTransactionModel({ chainId: chainId }),
      shortNameForChainId = pendingTransactionModelObject.defaultLongToShortNamesMap['chainId'],
      dataTypeForChainId = pendingTransactionModelObject.shortNameToDataType[shortNameForChainId],
      shortNameForTransactionHash = pendingTransactionModelObject.defaultLongToShortNamesMap['transactionHash'],
      dataTypeForTransactionHash = pendingTransactionModelObject.shortNameToDataType[shortNameForTransactionHash];

    for (let index = 0; index < transactionHashes.length; index++) {
      let queryParams = {
        TableName: pendingTransactionModelObject.tableName(),
        IndexName: pendingTransactionModelObject.firstGlobalSecondaryIndexName(),
        ...
        ExpressionAttributeValues: {
          ...
          ':txh': { [dataTypeForTransactionHash]: transactionHashes[index].toString() }
        }
      };

      promiseArray.push(pendingTransactionModelObject.ddbServiceObj.query(queryParams));
    }
    ...
    let formattedData = oThis._formatRowsFromDynamo(results, 'transactionHash');
  ...
  async clearCache(params) {
    const oThis = this,
      PendingTransactionCache = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'PendingTransactionCache'),
      pendingTransactionCache = new PendingTransactionCache({
        chainId: params.chainId,
        transactionUuids: [params.transactionUuid]
      });

    await pendingTransactionCache.clear();
  }
