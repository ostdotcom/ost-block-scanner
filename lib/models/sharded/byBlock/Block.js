...
class BlockModel extends ShardedBase {
  ...
  async setCache(dbResponse) {
    const oThis = this,
      ...
      cacheObj = new cacheKlass({
        ...
        blockNumbers: [dbResponse.blockNumber]
      });

    const requiredKeys = ['chainId', 'blockHash', 'gasUsed', 'blockNumber', 'totalTransactions', 'blockTimestamp'];
    let filteredBlockData = {};
    for (let index in requiredKeys) {
      if (dbResponse[requiredKeys[index]]) {
        filteredBlockData[requiredKeys[index]] = dbResponse[requiredKeys[index]];
      }
    }

    await cacheObj.setCache(dbResponse.blockNumber, filteredBlockData);
  }
  ...
  async getBlockDetails(blockNumbers) {
    ...
    let selectColumns = ['chainId', 'blockHash', 'gasUsed', 'blockNumber', 'totalTransactions', 'blockTimestamp'],
      projectionExpression = oThis._attributesProjection(selectColumns);
