...
class ShardByTransactionModel extends SharedBase {
  ...
  async getTransactionHashesByBlockNo(chainId, blockTimestamp, LastEvaluatedKey) {
    ...
    let transactionModelResponse = response.data.Items;
  ...
  async setCache(dbResponse) {
    const oThis = this,
      cacheKlass = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'ShardIdentifierByTransactionCache'),
   ...
   * @returns {Promise<*>}
   */
  async afterUpdate(queryResponse) {
    const oThis = this;
    ...
    const formattedResponse = oThis._formatRowFromDynamo(queryResponse.data.Attributes);
