...
class EconomyModel extends SharedBase {
   ...
   *
   * @returns {{contractAddress: string, chainId: string, name: string, symbol: string, totalSupply: string, createdTimestamp: string, updatedTimestamp: string}}
   */
  ...
  async getEconomyData(chainId, economyContractAddresses) {
    ...
    return Promise.resolve(responseHelper.successWithData(oThis._formatRowsFromDynamo(dbRows, 'contractAddress')));
  }
  ...
  async _fetchCompleteDataOfToken(queryResponses) {
    ...
    for (let index in queryResponses) {
      let queryResponse = queryResponses[index],
        dbRows = queryResponse.data.Items;
