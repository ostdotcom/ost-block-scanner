...
class EconomyModel extends SharedBase {
  ...
  thirdGlobalSecondaryIndexName() {
    ...
    return oThis.shortNameFor('sortEconomyBy') + '.' + oThis.shortNameFor('totalTokenHolders');
  }
    ...
    const oThis = this,
      tableSchema = {
        ...
        AttributeDefinitions: [
          ...
          {
            AttributeName: oThis.shortNameFor('totalTokenHolders'),
            AttributeType: 'N'
          }
        ],
  ...
  /**
   * This function performs a search on token symbol.
   *
   * @param {String} symbol
   *
   * @returns {Promise<any>}
  ...
  async searchBySymbol(symbol) {
    ...
    const shortNameOfSortEconomyBy = oThis.longToShortNamesMap['sortEconomyBy'],
      dataTypeOfSortEconomyBy = oThis.shortNameToDataType[shortNameOfSortEconomyBy],
      defaultValueOfSortEconomyBy = '1',
      shortNameOfTokenSymbol = oThis.longToShortNamesMap['symbol'],
      dataTypeOfTokenSymbol = oThis.shortNameToDataType[shortNameOfTokenSymbol],
      formattedSymbol = symbol.toString().toLowerCase();

    const tokenSymbolQueryParams = {
      TableName: oThis.tableName(),
      IndexName: oThis.secondGlobalSecondaryIndexName(),
      KeyConditionExpression: '#sortEconomyBy = :sortEconomyBy AND #tokenSymbol = :tokenSymbol',
      ExpressionAttributeNames: {
        '#sortEconomyBy': shortNameOfSortEconomyBy,
        '#tokenSymbol': shortNameOfTokenSymbol
      },
      ExpressionAttributeValues: {
        ':sortEconomyBy': { [dataTypeOfSortEconomyBy]: defaultValueOfSortEconomyBy },
        ':tokenSymbol': { [dataTypeOfTokenSymbol]: formattedSymbol }
      }
    };
