...
class ModelBaseKlass {
  ...
  /**
   * Get Merged Mapping of long column names to their short names.
   *
   * 1. uses default map defined by each model
   * 2. add extra mappings specified in config
   *
   * @return {*}
   * @private
   */
  _getMergedLongToShortNamesMap() {
    const oThis = this;

    let nameMap = oThis.defaultLongToShortNamesMap;
    ...
    for (let columnLongName in extraStorageColumns) {
      if (nameMap[columnLongName]) {
        problematicLongNames.push(columnLongName);
      } else {
        nameMap[columnLongName] = extraStorageColumns[columnLongName]['shortName'];
      }
    }
  ...
  /**
   * Things to do after record has been updated.
   *
   * @param updatedData
   */
  ...
  async putItem(rowMap) {
    ...
    let dbFormattedRow = oThis._formatDataForDynamo(rowMap),
      formattedData = {
        Item: dbFormattedRow,
    ...
    await oThis.afterUpdate(oThis._formatRowFromDynamo(dbFormattedRow));
    ...
    while (true) {
      ...
      for (let i = 0; i < batchedRawData.length; i++) {
        batchedFormattedData.push({
          PutRequest: {
            Item: oThis._formatDataForDynamo(batchedRawData[i])
          }
        });
      }
      ...
      if (
        ...
        for (let i = 0; i < allPromisesData.length; i++) {
          ...
          let batchItems = allPromisesData[i];
          batchedPromisesData.push(
            oThis.ddbServiceObj.batchWriteItem(batchItems).then(async function(response) {
              if (response.isSuccess()) {
                const afterUpdatePromises = [],
                  requestItems = batchItems.RequestItems[oThis.tableName()];
                for (let j = 0; j < requestItems.length; j++) {
                  const rowForDynamo = requestItems[j].PutRequest.Item;
                  afterUpdatePromises.push(oThis.afterUpdate(oThis._formatRowFromDynamo(rowForDynamo)));
                }
  ...
  async batchDeleteItem(rawData) {
    ...
    while (true) {
      ...
      if (batchedFormattedData.length > 0) {
        let batchWriteParams = { RequestItems: {} };
        batchWriteParams.RequestItems[oThis.tableName()] = batchedFormattedData;
        allPromisesData.push(batchWriteParams);
      }
  ...
  async updateItem(data) {
    ...
    formattedQuery['ReturnValues'] = 'ALL_NEW';
    ...
    let updateQueryResponse = await oThis.ddbServiceObj.updateItem(formattedQuery);
    if (updateQueryResponse.isFailure()) {
      return Promise.reject(updateQueryResponse);
    }

    const formattedResponse = oThis._formatRowFromDynamo(updateQueryResponse.data.Attributes);
    await oThis.afterUpdate(formattedResponse);
