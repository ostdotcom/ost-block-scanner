  ...
  async batchWriteItem(rawData) {
    ...
    while (true) {
      const offset = (dataBatchNo - 1) * batchWriteLimit,
        ...
        formattedRowsForDynamo = [],
        batchedFormattedData = [];

      for (let i = 0; i < batchedRawData.length; i++) {
        let rowData = batchedRawData[i],
          formattedRow = oThis._formatDataForDynamo(rowData);

        formattedRowsForDynamo.push(formattedRow);
      ...
      if (
        ...
        // Call After update for all successful batch writes
        const afterUpdatePromises = [];
        for (let i = 0; i < formattedRowsForDynamo.length; i++) {
          afterUpdatePromises.push(oThis.afterUpdate(oThis._formatRowFromDynamo(formattedRowsForDynamo[i])));
        }
