...
class WriteBase {
  ...
  async perform() {
    ...
    return responseHelper.successWithData({
      ...
      batchWriteFailed: oThis.batchWriteFailed
    });
  }
  ...
  _deleteAddressTransactionsData() {
    ...
    // Based on number of shards make entries in sharded tables parallelly.
    let insertPromises = [];
    for (let shardId in oThis.addressTrxsInsertData) {
      insertPromises.push(
        new Promise(function(onResolve, onReject) {
          oThis
            ._getModelToInsertData(shardId)
            .batchDeleteItem(oThis.addressTrxsInsertData[shardId])
            .then(function(resp) {
              if (!resp || resp.isFailure()) {
                oThis.batchWriteFailed = true;
                logger.error('economy address transactions deletion failed: ', resp);
              }
              onResolve();
            })
            .catch(function(err) {
              oThis.batchWriteFailed = true;
              logger.error('economy address transactions deletion failed: ', err);
              onResolve();
            });
        })
      );
