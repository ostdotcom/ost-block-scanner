...
class BaseMultiCacheManagement {
  ...
  async fetch() {
    ...
    while (data['cacheMiss'].length > 0) {
      ...
      for (let i = 0; i < cacheMissData.length; i++) {
        ...
        oThis.setCache(cacheMissFor, dataToSet);
      }
    }
   ...
   * @returns {Result}
   */
  async setCache(key, dataToSet) {
    const oThis = this;

    let setCacheFunction = function(k, v) {
      let cacheKey = oThis.invertedCacheKeys[k.toString()];
      logger.debug('Cache set for: ', cacheKey, ' Data to set: ', JSON.stringify(v));
  ...
  _cacheKeyPrefix() {
    const oThis = this;
    return oThis.ic().configStrategy.ddbTablePrefix + 'ost_bs_mm_';
  }
}
