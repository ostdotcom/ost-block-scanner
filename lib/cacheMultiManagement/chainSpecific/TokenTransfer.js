...
class TokenTransferCache extends BaseCache {
  ...
  /**
   * Set Cache of all transfer rows of transaction
   *
   * @param transferRows
   * @returns {Promise<void>}
   */
  async setTransfersCache(transferRows) {
    const oThis = this,
      promiseArray = [];

    for (let i = 0; i < transferRows.length; i++) {
      let row = transferRows[i],
        cacheKey = `${row.transactionHash}-${row.eventIndex}`;

      promiseArray.push(oThis.setCache(cacheKey, row));
    }
