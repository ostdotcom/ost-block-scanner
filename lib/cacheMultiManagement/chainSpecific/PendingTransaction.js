 ...
 *
 * @module lib/cacheMultiManagement/chainSpecific/PendingTransaction
 */
const rootPrefix = '../../..',
 ...
 * @class
 */
class PendingTransactionCache extends BaseCache {
  /**
  ...
  async fetchDataFromSource(cacheMissTransactionUuids) {
    ...
    for (let i = 0; i < cacheMissTransactionUuids.length; i++) {
      keyObjs.push({
        transactionUuid: cacheMissTransactionUuids[i],
        chainId: oThis.chainId.toString()
      });
    }

    let response = await pendingTransactionModelObject.getPendingTransactionData(keyObjs);
...
module.exports = PendingTransactionCache;
