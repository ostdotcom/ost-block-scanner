...
class RevertBlockTransactionsData {
  ...
  async cleanupTransactionsAndShards() {
    ...
    for (let shardId in oThis.shardTransactionsMap) {
      ...
      if (dataToDelete.length > 0) {
        await transactionModel.batchDeleteItem(dataToDelete, coreConstants.batchDeleteRetryCount);
        await transactionDetailModel.batchDeleteItem(dataToDelete, coreConstants.batchDeleteRetryCount);
      }
    }

    // Delete shard by transactions
    if (shardTransactionsToDelete.length > 0) {
      await shardByTransaction.batchDeleteItem(shardTransactionsToDelete, coreConstants.batchDeleteRetryCount);
    }
  }
  ...
  async cleanupTokenTransfers() {
    ...
    for (let shardId in oThis.shardTransactionsMap) {
      ...
      if (dataToDelete.length > 0) {
        await tokenTransferModel.batchDeleteItem(dataToDelete, coreConstants.batchDeleteRetryCount);
      }
    }
  }
