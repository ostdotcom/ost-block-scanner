...
class FetchBlockTransactions {
   ...
   * @constructor
   */
  constructor(chainId, blockNumber, blockTimestamp, fetchTokenTransfers) {
    const oThis = this;
  ...
  async asyncPerform() {
    ...
    let blockTrxResp = await oThis._fetchTransactionAndShards();
   ...
   * @returns {Promise<void>}
   */
  async _fetchTransactionAndShards() {
    const oThis = this;
