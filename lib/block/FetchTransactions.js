...
class FetchBlockTransactions {
  ...
  async asyncPerform() {
    ...
    if (oThis.newTransactionsOfBlock.length > 0) {
      let extraTransactionsToFetch = oThis.newTransactionsOfBlock.filter(function(i) {
        return oThis.transactionHashesFromDb.indexOf(i) < 0;
      });
      if (extraTransactionsToFetch.length > 0) {
        logger.info('Fetching Extra transactions of block to delete: ', extraTransactionsToFetch);
        let extraTrxResponse = await oThis._fetchTransactionShards(extraTransactionsToFetch, false);
        logger.info('Extra transactions found: ', extraTrxResponse.shardsFoundTx);
      }
    }
  ...
  async _fetchTransactionAndShardsOfBlock() {
    ...
    while (true) {
      ...
      if (transactionResponse.isSuccess()) {
        ...
        promisesArray.push(oThis._fetchTransactionShards(transactionHashes, true));
      } else {
   ...
   * @private
   */
  _fetchTransactionShards(transactionHashes, rejectIfNotFound) {
    const oThis = this,
      ShardIdentifierByTransactionCache = oThis
        .ic()
        .getShadowedClassFor(coreConstants.icNameSpace, 'ShardIdentifierByTransactionCache');
          ...
          if (getShardRsp.isSuccess()) {
            ...
            for (let i = 0; i < transactionHashes.length; i++) {
              ...
              if (shardIdentifier) {
                ...
                oThis.transactionHashesFromDb.push(txHash);
                ...
                if (rejectIfNotFound) {
                  return onReject(
                    responseHelper.error({
                      internal_error_identifier: 'l_b_ft_5',
                      api_error_identifier: 'something_went_wrong',
  ...
  async _fetchTransactions() {
    ...
    let promisesArray = [],
      TransactionCacheKlass = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'TransactionCache');
    ...
    for (let shardIdentifier in oThis.shardTransactionsMap) {
      ...
      while (true) {
        ...
        let promise = new TransactionCacheKlass({
          chainId: oThis.chainId,
          transactionHashes: batchedTransactions,
          consistentRead: oThis.consistentRead
        }).fetch();
  ...
  _sendBatchGetTransfers(shardIdentifier, transactionEventMap) {
    ...
    promise.then(function(resp) {
      ...
      for (let txHash in resp.data) {
        oThis.tokenTransfersData[txHash] = resp.data[txHash];
      }
    });
