...
class ConfigFormatter {
  ...
  /**
   * Get Data source config for given model name.
   *
   * @param modelName
   * @returns {*|string}
   */
  getDataSourceConfigFor(modelName) {
    const oThis = this,
      configStrategy = oThis.ic().configStrategy,
      nonDDBDataSource = configStrategy['nonDDBDataSource'] || {};

    return nonDDBDataSource[modelName] || 'ddb';
  }
}
